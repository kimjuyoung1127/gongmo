...
*   `[x]` **데모 시나리오 확정:**
    ...
    3.  [영수증] 영수증 촬영 → 3-5초 내 OCR 처리 → 자동 재고 등록
*   `[x]` **데모 영상 제작:** 시나리오별 데모 영상 제작 완료.
*   `[x]` **성과 보고서 작성:** 'AI + 바코드' 하이브리드 방식 및 데이터 중심 접근법의 성공 사례 강조.
*   `[x]` **README.md 및 문서 업데이트:** 실행 방법, API 명세, 문제 해결 과정(`progress.md`) 등 상세히 기술.
*   `[x]` **MLOps 기반 마련:** `Label Studio` 및 `MLflow` 초기 설정 (사용자 피드백 수집 및 실험 추적 환경 구축).
*   `[x]` **(제출물) `[1]성과보고서`, `[4]데모영상`, `[7]실행스크립트(README)`**

---

### 🔄 Week 5: 컴포넌트 모듈화 & 유지보수 (2025-11-15)

*   `[x]` **scan.tsx 파일 모듈화:** 대규모 scan.tsx 파일을 components/scan/ 디렉토리로 분리
    - `[x]` ModeToggle.tsx: 모드 전환 UI 컴포넌트 분리
    - `[x]` PhotoConfirmModal.tsx: 사진 확인 모달 컴포넌트 분리
    - `[x]` ScanUtils.ts: 공통 유틸리티 함수 및 타입 정의 분리
*   `[x]` **코드 구조 개선:** 기존 scan.tsx에서 새로운 모듈들로 연결
*   `[x]` **바코드 캐싱 시스템 완성:** products 테이블을 활용한 캐시 시스템 완성
*   `[x]` **ReceiptItemCard 컴포넌트 생성:** 영수증 항목 표시용 컴포넌트 추가
*   `[x]` **receipt-review.tsx 삭제:** 사용되지 않는 파일 정리
