...
## 🔧 프론트엔드 컴포넌트 모듈화 (2025-11-15)

### scan.tsx 파일 분리 완료
- **ModeToggle.tsx**: 바코드/영수증 모드 전환 UI 분리
- **PhotoConfirmModal.tsx**: 사진 확인 모달 UI 분리
- **ScanUtils.ts**: 공통 유틸리티 함수 및 타입 정의 분리
- **기존 코드 개선**: 기존 scan.tsx에서 새로운 모듈들로 연결하도록 업데이트
...
- [ ] **"최종 시스템 구조" 다이어그램 수정:**
  - [ ] `클로바 OCR 필터링` 단계를 `Clova OCR 텍스트 추출 -> Gemini LLM 상품명 추출`의 2단계로 더 명확하게 분리하여 표현.
- [ ] **"AI 기반 필터링 정교화" 섹션 업데이트:**
  - [ ] 현재의 복잡한 정규식 기반 필터링 로직(`_is_valid_product_item`)이 **LLM의 문맥 이해 능력으로 대체**되었음을 명시.
  - [ ] LLM이 가게정보, 금액, 날짜 등을 스스로 판단하여 제외함을 설명.
- [ ] **"필터링 파이프라인" 코드 예제 제거:**
  - [ ] `_is_valid_product_item` 함수 관련 내용이 더 이상 유효하지 않으므로, 해당 코드 블록을 제거하거나 'Legacy'로 표시.
- [ ] **"아키텍처" 섹션 업데이트:**
  - [ ] `사진 → 리사이즈 → Base64 → 클로바 API 호출 → 응답 수신` 이후의 흐름이 **LLM API 호출**로 이어짐을 명확하게 다이어그램 또는 텍스트로 추가.
