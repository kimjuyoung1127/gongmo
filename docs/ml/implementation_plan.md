...
## 🚀 Phase 7: 최종 아키텍처 진화 - LLM 도입 (2025-11-16)

- [ ] **전략 최종 수정:** 데이터 중심 접근법으로 61.59%의 정확도를 달성했으나, 여전히 실제 사용에는 부족하다고 판단. 특히, 비상품 텍스트(주소, 금액 등)를 완벽하게 필터링하지 못하는 한계에 직면.
- [ ] **LLM 기반 아키텍처로 완전 전환:**
  - [ ] OCR 품목 분류의 주체를 로컬 `scikit-learn` 모델에서 **외부 Gemini LLM API로 교체.**
  - [ ] **파이프라인 변경:** (Clova OCR 텍스트 추출) -> (**Gemini LLM 상품명 리스트 추출**) -> (규칙 기반 카테고리 분류)
- [ ] **기대 효과 및 최종 결과:**
  - [ ] **압도적인 정확도:** LLM의 강력한 문맥 이해 능력을 통해, '아메리카노'와 같은 실제 상품만 정확히 추출하고 나머지 모든 비상품 텍스트는 **완벽하게 필터링**하는 데 성공.
  - [ ] **로컬 모델 의존성 제거:** 더 이상 `.pkl` 모델 파일 관리, `scikit-learn`, `konlpy`, 데이터셋 관리, 재학습 파이프라인이 필요 없어짐.
  - [ ] **아키텍처 단순화:** 복잡했던 전처리 및 추론 코드가 LLM API 호출 하나로 통합되어 코드베이스가 훨씬 간결해짐.
- [ ] **최종 결론:** OCR 후처리 문제 해결에 있어, 특정 도메인에 맞춘 소규모 모델의 지속적인 튜닝보다 **강력한 범용 LLM을 활용하는 것이 압도적으로 효과적이고 효율적**임을 최종적으로 입증. 프로젝트의 핵심 OCR 처리 기능을 성공적으로 완성.
