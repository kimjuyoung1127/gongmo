...
- **처리 속도**: 20초 → 3초 (600% 향상)
- **정확도 향상**: 수동 파싱 → 네이버 AI 엔진
- **확장성 구조**: 모듈러화 설계로 추후 확장 용이
- **이미지 최적화**: 자동 리사이즈로 용량 압축

## 📋 시스템 구조

```mermaid
영수증 촬영 → 이미지 리사이즈 (2000px) → 클로바 OCR API → 필터링 → 데이터 구조화
```
...
### 개선된 필터링 로직
- **재고 관리 적합**: 금액, 날짜, 바코드, 카드 정보 제외
- **상품명 추출**: 상품명+수량 정교화
- **카테고리 분류**: 7개 대분류 + 상세 키워드 매핑
- **유통기한 예측**: 카테고리별 상세 유통기한 계산
