...
async def _extract_items_with_llm(full_text):
    ...
    try:
        ...
        print("[LLM] 프롬프트에 사용할 카테고리 목록을 DB에서 동적으로 로드합니다...")
        categories_info = ""
        etc_id = 37  # 기본 '기타' ID
        try:
            # 전역 범위에서 supabase 클라이언트를 사용합니다.
            response = supabase.table('categories').select('id, category_name_kr').order('id').execute()
            if response.data:
                categories_info_lines = [f"{cat['id']}: {cat['category_name_kr']}" for cat in response.data]
                categories_info = "\n".join(categories_info_lines)
                
                # '기타' 카테고리의 실제 ID를 찾습니다.
                etc_category = next((cat for cat in response.data if cat['category_name_kr'] == '기타'), None)
                if etc_category:
                    etc_id = etc_category['id']
                
                print(f"[LLM] 성공: DB에서 {len(response.data)}개의 카테고리를 로드했습니다. '기타' ID는 {etc_id}입니다.")
            else:
                raise Exception("DB에서 카테고리를 찾을 수 없습니다.")
        except Exception as e:
            print(f"[LLM-ERROR] DB에서 카테고리 로드 실패, 하드코딩된 목록으로 대체합니다. 오류: {e}")
            # DB 로드 실패 시, 기존의 하드코딩된 목록을 비상용으로 사용
            categories_info = """
            1: 유제품(신선) - 🥛, 2: 유제품(가공/롱라이프) - 🧈, 3: 연질치즈 - 🧀, 4: 경성치즈 - 🧀, 5: 육류(신선) - 🥩, 6: 가공육 - 🥓, 7: 난류 - 🥚, 8: 잎채소 - 🥬, 9: 줄기채소 - 🥒, 10: 뿌리채소(저온) - 🍠, 11: 발아채소 - 🌱, 12: 열매채소 - 🍅, 13: 버섯류 - 🍄, 14: 과일(일반) - 🍎, 15: 베리류 - 🍓, 16: 감귤류 - 🍊, 17: 열대과일 - 🥭, 18: 어류(신선) - 🐟, 19: 패류 - 🦞, 20: 연체/갑각류 - 🦀, 21: 해조류(생) - 🥬, 22: 해조류(건조) - 🥬, 23: 냉동식품 - ❄️, 24: 건면 - 🍝, 25: 생/냉장면 - 🍜, 26: 빵(일반) - 🍞, 27: 베이커리(크림/샌드) - 🥮, 28: 음료(냉장) - 🥤, 29: 음료(멸균/캔) - 🥤, 30: 과자/스낵 - 🍪, 31: 곡류/쌀 - 🌾, 32: 소스/조미료 - 🧂, 33: 김치/절임류 - 🥗, 34: 통조림/건식품 - 🥫, 35: 반조리/냉장 HMR - 🍽️, 36: 반조리/냉동 HMR - 🍽️, 37: 기타 - 📦
            """
            etc_id = 37
        # --- END DYNAMIC LOADING ---
        ...
        **규칙:**
        ...
        5. 카테고리는 위의 목록에서 가장 적절한 것을 선택하십시오. 정확한 매칭이 없으면 {etc_id}(기타)를 사용하십시오.
